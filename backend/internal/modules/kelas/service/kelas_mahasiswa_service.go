...
type (
	KelasMahasiswaService interface {
		Create(ctx context.Context, kelasID uuid.UUID, req dto.KelasMahasiswaCreateRequest) error
...
func (s *kelasMahasiswaService) Create(ctx context.Context, kelasID uuid.UUID, req dto.KelasMahasiswaCreateRequest) error {
	isExist, err := s.kelasRepo.CheckKelasById(ctx, s.db, kelasID)
	if err != nil {
	...
	for _, mahasiswaID := range req.MahasiswaID {
		isExist, err = s.userRepo.CheckByMahasiswaRoleAndDetailID(ctx, s.db, mahasiswaID)
		if err != nil {
			log.Printf("Internal Error: %v", err)
			return constants.ErrInternalErr
		}
		...
		isAlreadyAssigned, err := s.kelasMahasiswaRepo.CheckMahasiswaAlreadyAssigned(ctx, s.db, mahasiswaID)
		if err != nil {
			log.Printf("Internal Error: %v", err)
			return constants.ErrInternalErr
		}
	...
	assignEntitys := make([]entities.KelasMahasiswa, len(req.MahasiswaID))

	for i, mahasiswaID := range req.MahasiswaID {
		assignEntitys[i] = entities.KelasMahasiswa{
			KelasID:     kelasID,
			MahasiswaID: mahasiswaID,
		}
	}

	if err := s.kelasMahasiswaRepo.Create(ctx, s.db, assignEntitys); err != nil {
